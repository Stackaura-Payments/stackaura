...
export default function VoiceAgent({ onStateChange }: VoiceAgentProps) {
  ...
  function beginListening(sessionId: number) {
    ...
    if (!recognition) {
      setError(
        "This browser does not provide Speech Recognition. Use a supported Chromium browser.",
      );
      updateState("FAULT");
      return;
    }
  ...
  async function playFishStream(response: Response) {
    if (!response.body) {
      throw new Error("Fish Audio returned no audio stream.");
    }
  ...
  async function speak(text: string, sessionId: number) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        payload?.message ?? "Fish Audio speech synthesis failed.",
      );
    }
  ...
  async function sendToCore(text: string, sessionId: number) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.message ?? "JARVIS Core request failed.",
        );
      }
      ...
      await speak(
        payload.message ?? "J.A.R.V.I.S. completed the request.",
        sessionId,
      );
    } catch (cause) {
      processingRef.current = false;
  ...
  return (
    <section className="relative overflow-hidden border border-amber-400/[0.12] bg-black/55">
        ...
        <div className="bg-[#030302] px-4 py-4 sm:px-5">
          <div className="flex items-center gap-3">
            <div className="relative flex h-10 w-10 items-center justify-center rounded-full border border-amber-400/20 bg-amber-400/[0.035]">
              <div
                className={`h-2 w-2 rounded-full bg-amber-300 shadow-[0_0_18px_rgba(245,158,11,0.85)] ${
            ...
            <div>
              ...
              <div className="mt-1 font-mono text-[9px] uppercase tracking-[0.14em] text-amber-300/70">
                {state}
              </div>
            </div>
          </div>
        ...
        <div className="flex min-w-[180px] flex-col justify-center gap-2 bg-[#030302] px-4 py-4 sm:border-l sm:border-white/[0.045]">
          {!active ? (
            <button
              ...
              className="inline-flex items-center justify-center gap-2 border border-amber-400/20 bg-amber-400/[0.07] px-4 py-2.5 font-mono text-[8px] uppercase tracking-[0.16em] text-amber-200 transition hover:bg-amber-400/[0.13]"
            >
            ...
            <>
              <button
                onClick={toggleMute}
                className="inline-flex items-center justify-center gap-2 border border-white/[0.08] bg-white/[0.025] px-4 py-2.5 font-mono text-[8px] uppercase tracking-[0.16em] text-white/55 transition hover:bg-white/[0.05]"
              >
                {muted ? (
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 border border-red-400/10 bg-red-400/[0.025] px-4 py-2.5 font-mono text-[8px] uppercase tracking-[0.16em] text-red-300/65 transition hover:bg-red-400/[0.06]"
              >
      ...
      <div className="relative flex flex-wrap gap-x-5 gap-y-2 border-t border-white/[0.05] px-4 py-2.5 font-mono text-[7px] uppercase tracking-[0.14em] text-white/20 sm:px-5">
        <span>
          Provider:{" "}
          <b className="font-normal text-amber-400/45">Fish Audio</b>
        </span>
        <span>
          Model:{" "}
          <b className="font-normal text-amber-400/45">{VOICE_MODEL}</b>
        </span>
        <span>
          Voice:{" "}
          <b className="font-normal text-amber-400/45">
            {VOICE_ID.slice(0, 8)}…
          </b>
        </span>
        <span>
          Authority:{" "}
          <b className="font-normal text-amber-400/45">JARVIS CORE</b>
        </span>
      </div>
    </section>
  );
